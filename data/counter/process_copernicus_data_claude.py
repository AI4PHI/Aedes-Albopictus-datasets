...
def extract_climate_data(ds, variable, lat, lon, start_date, end_date):
    ...
    # Extract time range - handle both 'time' and 'valid_time' dimensions
    if 'valid_time' in ds.dims:
        time_dim = 'valid_time'
    elif 'time' in ds.dims:
        time_dim = 'time'
    else:
        raise ValueError("Dataset must have either 'time' or 'valid_time' dimension")

    print(f"  🕒 Using time dimension: '{time_dim}' for extraction")

    # If variable has both time and valid_time dimensions, select first time slice and then slice valid_time
    #if 'time' in ds.dims and 'valid_time' in ds.dims and time_dim == 'valid_time':
    #    # Select first time slice to reduce to 3D, then slice valid_time
    #    ds = ds.isel(time=0)  # Take first monthly slice
    #    print(f"  📅 Reduced 4D to 3D by taking first monthly slice")
    ...
    print(f"  🌍 Latitude range in data: {lats.min()} to {lats.max()}")
    ...
    print(f"  📍 Surrounding grid points for ({lat}, {lon}):")
    ...
    print(f"    ({lat1}, {lon1}), ({lat2}, {lon1}), ({lat1}, {lon2}), ({lat2}, {lon2})")
    ...
    # Return results as a dictionary (time series)
